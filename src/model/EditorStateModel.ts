...
export class EditorStateModel {
  ...
  loadConfig(documentUri: string): EditorConfig | undefined {
    const raw = this.context.workspaceState.get<EditorConfig | RegexGroup[]>(`log-minus-minus.config.${documentUri}`);
  ...
  saveConfig(documentUri: string, config: EditorConfig): void {
    this.context.workspaceState.update(`log-minus-minus.config.${documentUri}`, config);
  }
}
